...
- スキーマ変更は Supabase の SQL Editor で実行する。Claude Codeから直接
  ...
  （スキーマ変更・SQL実行を指す。読み取り専用のREST API呼び出しについては
  別途、下記「本番シークレットの取り扱い」節を参照）。
...
### 本番シークレットの取り扱い（2026年9月15日制定）

2026年9月15日、本番DB調査作業中に`SUPABASE_SERVICE_ROLE_KEY`を`vercel env pull`で
ローカルファイルに書き出し、本番DBへの直接接続に使おうとした事例が発生した
（安全機構にブロックされ中止、ファイルは即座に削除し残留が無いことを確認済み。
実害無し）。「production migrationsは人間実行のみ」の既存方針の延長として、
以下を明文化する。以下は上記「人間の手動実行が必須の操作」とは別カテゴリの
制約（実行の可否ではなく、機密情報の取り扱いに関する禁止事項）である。

- service_roleキー・webhook secret（`APPLE_HEALTH_SYNC_SECRET`等）といった
  本番シークレットは、**調査目的であっても**ローカルファイルに書き出したり、
  本番DB・本番APIへの直接接続に使用したりしない（`vercel env pull`で本番環境の
  シークレットを取得すること自体を含む）。
- 本番DBへの読み取り専用の調査が必要な場合、claude-in-chromeで本番環境に
  ログイン済みのブラウザセッションから取得した認証済みセッショントークン
  （`authenticated`ロール、RLS適用済み）経由でSupabaseのREST APIを叩く方式は
  使ってよいが、**実行前に必ずこのチャットへ一声かけること**（read-onlyの
  診断エンドポイントであってもチャットでの確認が必須、という既存の学びを
  ここでも適用する。「または」ではなく、実行前確認が前提の手順である）。
- 上記に反した場合は、たとえ実害が無くても（今回のように未然にブロックされた
  場合を含む）、その旨をJohnさんへ必ず報告すること。
...
- フェッチ処理のcatch節でloaded系のフラグを安易にtrueにしない。失敗時はfalseのまま
  維持し、エラー通知のみ行う（同上）。
- カレンダー閲覧専用化（Phase C、2026年8月16日）以降、「編集用フォーム」
  （`ScheduleForm.tsx`等）と「閲覧用の日別サマリー表示」（`CalendarDaySummaries.tsx`）
  が別コンポーネントに分かれている。予定・実績・食事記録等に表示項目を追加する際は、
  編集フォーム側だけでなく閲覧用サマリー側も更新が必要か確認すること（2026年8月18日、
  スプリント3で試合種別表示が編集フォームにのみ反映され閲覧用サマリーに反映されて
  いなかった見落としが発生）。
- `exercises`テーブルへ新規INSERTする際は`user_id`列を明示的に指定すること
  （2026年8月19日、AI一括取り込み欠落対応時の教訓）。`exercises`は
  `is_preset`/`user_id`の組み合わせで取得条件が決まる設計（`fetchExercises`の
  `.or('is_preset.eq.true,user_id.eq.${DEFAULT_USER_ID}')`）のため、`user_id`列に
  NOT NULLのデフォルト値が設定されておらず、`NULL`のまま登録すると`is_preset=false`
  の行は取得条件のどちらにも一致せず、`is_deleted=false`であってもアプリの種目
  選択UIから一切表示されなくなる。`food_items`・`meal_sizes`の「`NULL`=共有」方式
  とは扱いが異なる点に注意（詳細は下記「2026年8月19日の変更点」参照）。
- 種目・食材等のマスタに新規追加する前に、類似の既存名がないか確認すること
  （2026年8月19日の教訓）。表記ゆれによる重複登録（例：「シーテッドロー」と
  「シーテッドロウ」）は、追加登録した時点では気づきにくく、事後の
  `training_log_exercises.exercise_id`付け替え＋論理削除といった復旧作業が
  必要になる。
...
- 【2026年8月23日解消】以前は本番URLへの直接アクセス（バンドルハッシュの
  比較等）でしかデプロイ反映を間接的に確認できなかったが、`vercel` CLIを
  devDependencyに導入したことで、`vercel ls`（直近デプロイ一覧・status）・
  `vercel inspect <url>`（個別デプロイの詳細）をClaude Codeから直接実行して
  デプロイ状況を確認できるようになった。ただし、Claude Code公式のVercel
  プラグイン（`.claude/plugins`にインストール済み）に含まれる`/vercel:status`
  等のSkillは、本セッションでは`Unknown skill`エラーとなり呼び出せなかった
  （原因未特定）。実際に機能を確認できたのは素の`vercel` CLIコマンドの方で、
  プラグイン経由のスラッシュコマンドが使えるかは別セッションでの再確認が必要
  （推測で「プラグインも使える」とは記載しない）。

### 品質基準・自律修正ループ（Exit Criteria、2026年8月23日制定、同日改定）

タスク完了を人間に報告する前に、以下を自動実行すること。コード改修から
報告までの流れは「①vitest→②build→③（グリーンなら）ローカルコミット→
④ブラウザ確認は別セッション→⑤完了報告（デプロイ確認を含む）」の順とする。

1. `npx vitest run`（`src/utils/`配下のユニットテスト）と `npm run build`
   （TypeScript型チェック＋Viteビルド）を実行する。エラーが出た場合は
   エラーログを解析し、コードを修正して再実行する自律修正ループを
   最大5回まで行ってよい。この2つはローカルで完結し本番・リモート状態に
   一切影響しないため、失敗のたびに人間へ確認を求める必要はない。
2. 5回試行してもグリーンにならない場合はループを止め、以下3点を含む
   エスカレーションレポートを人間に提出すること（無限に繰り返さない・
   黙って諦めない）：①試した修正内容、②直近のエラーログとエラー理由、
   ③人間の判断が必要な理由。
3. `vitest`・`build`が両方グリーンになったら、変更内容をローカルに
   コミットする（`git add`＋`git commit`。2026年8月23日改定：以前は
   コミットも都度人間の指示を受けてから実行する方針だったが、この自律
   ループ内で完結する範囲に限り自動化した）。**pushは自動化しない。**
   pushは他者から見える状態変更であり、本番と開発が同一Supabaseプロジェクト
   であることも踏まえ、これまで通り人間の明示的な指示を都度受けてから
   実行する。
4. claude-in-chromeによるブラウザ動作確認は、上記「開発方針」の
   「ブラウザでの動作確認はコーディング作業と別セッションで行う」との
   既存ルール通り、`/clear`後の別セッションで分離実行する。この自律修正
   ループのゲートには含めない（同一セッション内でコード変更直後にブラウザ
   確認まで自動連結すると、コンテキストの肥大化を招き既存ルールとも
   矛盾するため）。
5. Vercel CLI（`vercel ls`/`vercel inspect`等、上記「デプロイ確認」参照）
   によるデプロイ状況の確認は、1〜4の自律修正ゲートの一部ではなく、
   人間への完了報告に含める最終確認ステップとして実施する（コード品質の
   Exit Criteriaとは別の関心事のため、同一ループに混ぜない）。

### 人間の手動実行が必須の操作（2026年8月23日整理）

自律化・自動化を進めた結果、現時点でClaude Codeが代行できず人間の手動実行が
必須なのは以下の2点のみ。

1. **Supabase本番データベースへのマイグレーション適用**（`supabase db push`
   実行、またはSQL Editorでの実行）。
   【理由】本番と開発が同一Supabaseプロジェクトであるため、誤ったスキーマ
   書き換えが即座に本番データへ反映される。Claude Codeが生成したマイグレー
   ションSQLは人間が最終プレビューし、実行そのものは必ず人間が行うことで
   本番データ全損を防ぐ安全策とする（上記「Supabase」節の既存ルールと同一）。
2. **Vercel／GitHubの認証系コマンド**（`vercel login`・`vercel link`・
   GitHubの初回認証等、アクセストークンや秘密情報の発行・入力を伴うもの）。
   【理由】認証情報を扱う操作であり、Claude Codeが直接取り扱ってはならない
   （本ドキュメント全体の安全原則）。

上記2点以外（マイグレーションSQLファイルの作成・vitest/buildの実行・
ローカルコミット・デプロイ状況確認等）はClaude Codeが自律的に実行してよい。
...
【2026年8月28日改定】既定値はダーク固定（旧実装のOS設定`prefers-color-scheme`への
追従は廃止）。`localStorage`（`workout-app:theme`キー）に選択したテーマを保存し、
リロード後も手動で選んだテーマを維持する（保存値が無い初回アクセス時のみダークを
既定値として使う）。
...
自動連携（Phase B）は、本番環境に`training_templates`データが存在せず当時は
実機能検証が保留となっていたが、後述のテンプレート管理UI実装により
本番環境でも実機能検証済みとなった。

**テンプレート管理UI**（`src/components/TrainingTemplateManager.tsx`新規、
`Settings.tsx`に組み込み）：Phase Fで廃止されて以降UI上に存在しなかった
`training_templates`の新規レコード作成手段を新設した（Phase Fで廃止したのは
「実績記録時にその場でテンプレートを適用する」機能であり、テンプレート自体の
作成・一覧・削除は別機能として実装。技術的負債#7として記録していた項目の
解消）。一覧表示（テンプレート名・種目数）・新規作成（テンプレート名＋既存の
`ExercisePicker.tsx`（部位→種目の2段階選択）を種目スロットの配列として繰り返し
使う形で複数種目を選択、`ExercisePicker`自体のインターフェースは無変更）・
削除（`training_templates`に`is_deleted`列は存在しないため物理削除。
`training_template_exercises`はON DELETE CASCADEで自動削除、
`training_schedules.template_id`はON DELETE SET NULLのため、削除済み
テンプレートを参照していた既存の予定は、予定自体は削除されず参照のみが外れる
設計であることを本番環境で実際に確認済み：テンプレート参照ありの予定を作成
→テンプレート削除→予定のタイトル・絵文字・ステータスは保持されたまま
テンプレート選択のみ「選択しない」にリセットされることを確認した）に対応。

「予定を作成」ボタン（テンプレート一覧の各行）は、新しいクロスページ状態管理を
作らず、既存の`RecordModalRequest`／`openRecordModal`（App.tsx直下でグローバルに
開ける`RecordFormModal`の仕組み、元々カレンダー画面用）をそのまま延長する設計
にした（`RecordModalRequest`に`templateId`を追加し、`ScheduleForm.tsx`まで
受け渡す）。判断理由：`Settings.tsx`は日付選択の文脈を持たないカレンダー外の
画面のため、新規のクロスページ状態管理を作るより、既存の仕組みを延長する方が
変更範囲が小さく安全と判断した。予定の日付は常に本日（todayString）固定。
`ScheduleForm.tsx`側では、テンプレート一覧が非同期取得のため、`autoOpenToken`と
`templates`の両方を監視し1回のトークンにつき一度だけテンプレートを自動選択する
`useEffect`を追加した（新規予定作成時のみ、既存予定の編集オープンとは併用しない）。

**goalsの削除機能**（`src/api/goals.ts`の`deleteGoalByMonth`新設、
`GoalPanel.tsx`）：技術的負債#8として記録していた「テスト等で作成した不要な
月次目標データを削除する手段がない」問題を解消した。`goals`テーブルに
`is_deleted`列は存在しないため物理削除とした。削除ボタンは過去月かつ実データが
ある場合（`!isCurrentMonth && historyGoals`）のみ表示し、**当月は削除ボタン
自体を描画しない**（当月のgoalsを誤って削除するとDashboardの進捗比較カード等に
影響するため）。削除確認ダイアログは対象年月を具体的に表示する（例：
「2026年1月の目標を削除しますか？」）。削除成功後は月一覧から対象月を除外し、
表示を当月に自動的に戻す。

副次的な成果として、本機能の動作確認時に、以前のセッション（goals過去月一覧
表示UIの動作確認、削除機能実装前）から残存していた2026年1月分のテストデータ
（55.5kg）を実際に削除でき、既知の技術的負債として記録していた残存データの
問題も解消された。

**スプリント3：試合日（MD）基準の栄養・トレーニング調整**：9月目標（サッカー
コンディションのピーク化）に向け、試合日（Match Day、MD）を起点としたピリオダイ
ゼーション（負荷・栄養の動的調整）機能を追加した。

- `training_schedules`に`schedule_type`列（`match`/`practice`/`event`、
  デフォルト`practice`）を追加。予定作成時の必須選択は求めず、試合の予定を
  作る時だけユーザーが手動で「試合」を選択する運用。
- `src/utils/periodizationHelpers.ts`（新設）：`MD_ADJUSTMENT_TABLE`
  （MD-3〜MD+1のカロリー・タンパク質・脂質補正率）、`getMatchDayStatus`
  （直近の試合予定からMDステータスを判定）、`calculateAdjustedGoals`
  （PFC目標の逆算方式：カロリー目標を最優先しタンパク質・脂質を確定させた後、
  残りのエネルギーをすべて炭水化物に変換）。ACWR・移動平均と同じくDBキャッシュ
  なしで動的計算する方針を踏襲。
- ダッシュボードのカロリーリングに、MDバッジ（「MD-1」「MATCH DAY」等、範囲外は
  非表示）と、補正後の`calorieTarget`・`proteinTarget`・`carbsTarget`・
  `fatTarget`を分母として表示。補正時は「試合前調整 +15%」等のインジケーターを
  添える。`selectedDateKey`基準（カロリーリング自体と同じ基準。`ACWRGaugeCard`の
  ような`todayString`固定カードとは異なる扱い）。MD判定には対象日の前日〜3日後を
  含む窓の予定データが必要なため、週間ストリップ用の週スコープ取得とは独立した
  専用の取得窓を設けた（週境界付近で判定を誤るリスクを回避するため）。
- 筋トレ記録フォーム（`TrainingLogForm.tsx`）に、選択種目の部位が「脚」かつ
  当日の判定がMD-2/MD-1/MDの場合のインライン警告を追加（例：「⚠️ 明日は
  試合日（MD-1）です。下半身（脚）の高負荷トレーニングは疲労蓄積を防ぐため
  控えめを推奨します」）。**記録の保存自体は妨げない**（警告のみ）。

**実装中に発見・修正したバグ2件**：

1. **補正なし時（MD範囲外）の炭水化物量の誤り**：`calculateAdjustedGoals`が、
   MD補正が発動していない通常の日でも、カロリー・タンパク質・脂質の合計から
   炭水化物量を逆算した値を返していたため、Settingsで設定した実際の値と
   表示値が一致しないケースがあった（デフォルト設定値では逆算すると265gに
   なるが、実際の設定値は250g）。逆算方式はMD補正時専用のロジックとし、
   補正が発動していない日は設定値をそのまま返すよう修正した。本番環境で
   Settings側の設定値と表示値が完全一致することを確認済み。
2. **試合種別表示が閲覧用サマリーに反映されていなかった問題**：`ScheduleForm.tsx`
   （編集モーダル内の一覧）には試合種別のサフィックス表示（「・試合」）を
   追加したが、`CalendarDaySummaries.tsx`のScheduleSummary（MonthlyCalendarの
   閲覧専用サマリー表示、Phase C・2026年8月16日で新設された別コンポーネント）
   には同じ更新が漏れており、カレンダー上の一覧では試合種別が見えない状態
   だった。データ自体は正しく、表示ロジックの重複漏れが原因。同じ表示条件を
   `CalendarDaySummaries.tsx`にも追加して解消した（このカレンダー閲覧専用化に
   起因する「編集フォームと閲覧用サマリーが別コンポーネント」という構造上の
   留意点は、上記「開発方針」に一般則として追記した）。

本番環境（workout-app-suke4.vercel.app）で、試合予定作成時のカレンダー表示
（「・試合」表示、修正後）、MD-1〜MD+1各ステータスでのバッジ・補正値（設定値との
乗算が正確であること）、MD範囲外の日での未補正値がSettingsの設定値と完全一致
すること（バグ修正の確認）、週境界をまたぐ場合のMD判定、脚種目でのMD-2/MD-1/MD
警告表示と保存が妨げられないこと、ダーク/ライト両モードを確認済み。

## 2026年8月19日の変更点

**テンプレート編集機能（Phase A）**：`TrainingTemplateManager.tsx`（2026年8月18日
新設）はテンプレートの新規作成・一覧・削除のみに対応していたが、名前・種目構成の
フル編集にも対応した。テンプレート一覧の各行に「編集」ボタンを追加し、タップすると
新規作成と同じフォーム（テンプレート名入力＋`ExercisePicker`による種目複数追加）に
既存の値（テンプレート名・登録済み種目一覧）を初期表示した状態で開く。保存時は
`editingTemplateId`の有無で`createTrainingTemplate`／新設の`updateTrainingTemplate`
（`src/api/trainingTemplates.ts`）を使い分ける。`updateTrainingTemplate`は
`training_templates.name`をUPDATEし、`training_template_exercises`は対象
`templateId`分のみ全削除してから再登録する（新規作成時と同じ挿入ロジックを流用）。
`training_templates`本体（IDそのもの）は削除しないため、`training_schedules.template_id`
の参照には影響しない。

**テンプレート編集時の既存予定への反映（Phase B：調査→見送り）**：テンプレートを
編集した場合、それを参照している既存の予定（`training_schedules.template_id`）に
変更を反映すべきか調査した。調査の結果、`ScheduleForm.tsx`の`handleTemplateChange`／
`buildAutoTitle`（2026年8月18日実装）は、テンプレート選択時点で生成した文字列を
`training_schedules.title`（TEXT列）へそのまま書き込む「作成時コピー確定方式」
であり、動的参照ではないことを確認した。また、`training_schedules`にはタイトルが
「自動入力のまま」か「ユーザーが手動編集済み」かを区別する追跡列が一切存在しない
ことも確認した。この状態で反映処理を実装すると、ユーザーが意図的に手直しした
タイトルを誤って上書きしてしまうリスクを排除できないため、実装を見送った
（詳細は下記「既知の技術的負債」1番参照）。

本番環境（workout-app-suke4.vercel.app）で、編集ボタンからの既存値の初期表示、
名前・種目構成を変更しての保存と一覧への即時反映、**テンプレート編集後も
それを参照する既存の予定のタイトル等が変化しないこと**（子テーブル入れ替えが
既存の予定に影響しないことの確認、最重要ポイント）、編集後に「予定を作成」した
新規の予定には更新後の内容が反映されること、新規作成フローへのリグレッションが
ないことを確認済み。

**AI一括取り込みでのトレーニング記録欠落（2026-08-02〜08-17、9日分）と対応**：
AI一括取り込みで一部のトレーニング記録が登録されない事象が発生し、原因調査の
うえで手動再取り込みにより解消した。

- **原因1（種目マスタの不足）**：バイセップ・バイセップスカール・
  トライセップ・トライセプスプレス・トーソローテーション・ハックスクワット・
  ヒップアダクション・ヒップアブダクション・アブドミナルクランチ・シーテッドロー
  等11件がexercisesマスタに未登録で、AI一括取り込みが名称一致できずスキップ
  していた。
- **原因2（表記ゆれ）**：JSON側の種目名（「ラット・プルダウン」「クランチ台」
  「ペクトラルフライ」「ペックフライ・リアデルトフライ」）がマスタの正式名称
  （「ラットプルダウン」「クランチ」「ペックフライ」）と一致しておらず、
  `matchByNameWithFallback`のフォールバック（末尾括弧書き除去）でも救えなかった。
- **二次的な不具合（新規追加した種目がUIに一切表示されない）**：不足していた
  11件を手動SQLでINSERTしたところ、追加した種目が種目選択UIの候補一覧に
  まったく表示されなかった。調査の結果、`exercises.user_id`列にはNOT NULLの
  デフォルト値が設定されておらず、手動INSERT時にuser_idを明示しなかったため
  `NULL`のまま登録されていたことが原因と判明した。`fetchExercises`
  （`src/api/trainingLogs.ts`）は
  `.or('is_preset.eq.true,user_id.eq.${DEFAULT_USER_ID}')`で絞り込むため、
  `user_id`が`NULL`かつ`is_preset`が`false`の行はこのOR条件のどちらにも一致
  せず、`is_deleted=false`であっても取得結果から漏れていた（PWA/Service
  Workerのキャッシュや`is_deleted`の設定ミスではなく、あくまで`user_id`が
  原因であることをブラウザ側の検証で切り分けた）。該当11件の`user_id`を
  `DEFAULT_USER_ID`に更新することで解消した。
- **手動再取り込み**：表記ゆれ3件をデータ側で事前に統一したうえで、
  既存の登録内容と重複しないよう未登録の種目のみを個別CRUD方式で
  claude-in-chrome経由で追加した（09-09のみ実績自体が未作成だったため新規
  作成）。1日分ずつ、既存の登録内容を実際にスクリーンショットで確認してから
  不足分のみ追加する方式を徹底し、途中で見つかった重複記録（08-15で自動処理の
  過程で生じたベンチプレスの二重登録）もその場で削除して解消した。
- **「シーテッドロー」と「シーテッドロウ」の重複解消**：手動再取り込みの過程で
  「シーテッドロー」を新規種目として追加したが、後日これが既存の「シーテッドロウ」
  と同一種目の重複（表記ゆれ）であったことが判明した。データ損失を避けるため、
  `training_log_exercises`は`exercise_id`+`order_index`のみを持ち、実際の
  セットデータ（重量・回数）は`training_sets`が`training_log_exercise_id`で
  紐づいている設計を活かし、08-12分の`training_log_exercises.exercise_id`を
  既存「シーテッドロウ」のIDへ付け替えたうえで、重複していた「シーテッドロー」
  のexercisesレコードを論理削除（`is_deleted = true`）する方式で解消した
  （`training_sets`自体は一切変更しないため、セットの重量・回数データは
  失われない）。

本番環境（workout-app-suke4.vercel.app）で、9日分すべての種目・セット数・
重量・回数が正しく登録されていること、種目単体削除・重複解消後もカレンダー
表示・進捗グラフに欠落や重複がないこと、「シーテッドロー」→「シーテッドロウ」
の付け替え後もセットデータ（重量・回数）が失われていないことを確認済み。

## 2026年8月19〜20日：AI一括取り込み・マスタ登録時の類似名確認ダイアログ機能

上記「シーテッドロー」/「シーテッドロウ」重複事故（既知の技術的負債5番）の
再発防止のため、表記ゆれによる類似名を検知して確認を求めるダイアログ機能を
実装した。

- **Phase A（類似度判定ロジック）**：`src/utils/nameMatching.ts`に
  レーベンシュタイン距離ベースの`calculateNameSimilarity`（0〜1に正規化）・
  `findMostSimilarName`（マスタ内の最類似候補を1件返す、閾値`NAME_SIMILARITY_THRESHOLD
  = 0.6`未満はnull）を新設。外部ライブラリは新規導入せず軽量な自前実装とした。
  従来`BulkScheduleImportModal.tsx`にローカル定義されていた
  `matchByNameWithFallback`/`stripTrailingParenthetical`（完全一致→末尾括弧書き
  除去のフォールバック）もこのファイルへ移設（挙動は無変更）。
- **Phase B（AI一括取り込みプレビューへの確認UI）**：`BulkScheduleImportModal.tsx`の
  `ParsedExercise`/`ParsedMeal['items']`に`suggestedMatch: { id; name } | null`を追加。
  完全一致・フォールバックでも一致しなかった名前に対し、プレビュー内で
  「『◯◯』は『△△』と同じ種目/食材ですか？」＋「はい（同じ種目として取り込む）」
  「いいえ（別の種目として扱う）」を表示する。
- **Phase C（マスタ新規登録時の確認）**：`ExerciseNameInput.tsx`（`TrainingLogForm.tsx`
  内）・`MealLogForm.tsx`の「新しい食材をここで登録」に、登録前の類似名チェックを
  追加。類似候補があれば「『◯◯』という類似の種目/食材『△△』が既に存在します。
  それでも新規登録しますか？」＋「はい（新規登録する）」「いいえ（『△△』を使う）」
  を表示し、「いいえ」を選ぶと既存のマスタ項目に切り替わる。
- スコープ外（判断理由）：複数候補選択UI・一括Yes/No・エイリアス永続化記憶機能は
  実装指示書の時点で見送り。

各Phase完了ごとにビルド確認（`tsc --noEmit`・`npm run build`）・コミット
（`26b7ec8`Phase A・`d5c5422`Phase B・`8c85d22`Phase C）を行い、pushは全Phase
完了後に1回実施。

**本番環境（workout-app-suke4.vercel.app）での動作確認（2026年8月20日、claude-in-chrome
使用、別セッションで実施）**：類似度0.6以上のペア（クランチ台/クランチ=0.8、
シーテッドロー/シーテッドロウ=0.86、ほうれんそう/ほうれん草=0.67、サラダー/
サラダ=0.75）と、既存マスタと無関係な新規名（検証用種目Q・検証用食材Q、
類似度0.0）を使い分けて以下を確認した。

1. AI一括取り込みで「クランチ台」を含むJSONを取り込むと確認ダイアログが表示される
2. 「はい」で「クランチ」として正しく登録される
3. 「いいえ」で従来通りスキップされる（未取り込み）
4. 完全一致する項目（ベンチプレス・卵等）には確認が出ない（リグレッションなし）
5. 食材名でも同様の確認が機能する（ほうれんそう→ほうれん草）
6. **`ExerciseNameInput`（`TrainingLogForm.tsx`）で「シーテッドロー」入力時に
   既存の類似種目「シーテッドロウ」への確認が出て、「いいえ」で既存種目に
   正しく切り替わる**（今回の重複事故と同一状況の再現確認、最重要ポイント）
7. `MealLogForm.tsx`の食材新規登録でも同様に機能する（「サラダー」→「いいえ」で
   既存の「サラダ」を使用／「ほうれんそう」→「はい」で新規登録の両方を確認）
8. 通常の重複しない新規名（検証用種目Q・検証用食材Q）では確認が出ずに
   リグレッションなく登録できる

テスト用データ（検証用種目Q・検証用食材Q、および試験過程で一時的に作成した
選択・入力状態）はすべて確認後に削除・破棄済み。2026-08-21のトレーニング実績
...
## 2026年8月23日：開発基盤整備（Supabase CLI・Vitest・Vercel CLI・ConfirmDialog）

「確認・手戻りゼロ」に向けた開発基盤の整備を実施した。個々の判断理由は
上記の各節（Supabase・デプロイ確認・Exit Criteria・人間の手動実行が必須の
操作）および下記「既知の技術的負債」6番に集約済みのため、ここでは全体の
経緯のみ記録する。

- **Supabase CLI導入**：`supabase`をdevDependencyに追加し、`supabase init`で
  ローカル設定（`supabase/config.toml`）を生成した。`supabase link`・
  `db push`・`db pull`は認証情報を要するため未実行のまま（上記「人間の
  手動実行が必須の操作」参照）。`supabase/migrations/`にREADMEを配置し、
  初回のベースラインマイグレーションは`db pull`で本番DBから正確に生成する
  方針とした（手動での682行分のSQL履歴からのCREATE TABLE再構成は、
  見落とし・型の取り違えのリスクがあるため見送った）。
- **Vitest導入**：`src/utils/`配下の純粋関数（`chartHelpers`・
  `calendarHelpers`・`prHelpers`・`streakHelpers`・`acwrHelpers`・
  `recoveryHelpers`・`nameMatching`・`periodizationHelpers`）に対し、
  境界値中心のユニットテストを142件追加した（`vitest.config.ts`は
  `vite.config.ts`と独立させ、既存ビルド設定への影響を避けた）。
- **window.confirm()の全廃**：Native Dialogがclaude-in-chromeのブラウザ
  自動操作をブロック・フリーズさせる問題があったため、`src/hooks/useConfirm.tsx`
  （`ConfirmProvider`／`useConfirm`）と`src/components/common/ConfirmDialog.tsx`
  を新設し、`window.confirm()`を使用していた10ファイル11箇所すべてを
  `await confirm(message)`に置き換えた。**【解消済み・2026年8月23日】**
  claude-in-chromeで7種類の削除フロー（テンプレート・目標・予定・
  トレーニング実績（種目単位・日単位）・食事記録・サッカー記録・体調記録）
  全てを実際に操作し、ブロッキング・フリーズが一切発生しないことを確認済み。
  オーバーレイタップでのキャンセル・危険色ボタン（`.btn-danger`）・
  ダーク/ライト両モードでの表示も確認済み。
- **Vercel CLI導入**：`vercel`をdevDependencyに追加し、`vercel ls`/
  `vercel inspect`によるデプロイ状況確認が可能になった
  （詳細は上記「デプロイ確認」参照）。導入時のnpm audit脆弱性への対応は
  下記「既知の技術的負債」6番を参照。
- **Exit Criteria運用ルールの制定・改定**：上記「品質基準・自律修正ループ」
  参照。

## 2026年8月28日：ダークモードの既定値変更

`src/hooks/useTheme.ts`の初期テーマ判定を、OS設定（`prefers-color-scheme`）への
追従からダーク固定に変更した。あわせて`localStorage`（`workout-app:theme`キー、
`deviceId.ts`と同じ命名規則）への永続化を新設し、切替ボタンで選んだテーマが
リロード後も維持されるようにした（保存値が無い初回アクセス時のみダークを使う）。

**調査結果（変更前の状態）**：テーマは`profiles`テーブル等のDBには一切保存されて
おらず、単なるReactの`useState`のみで管理されていた（初期値をOS設定から動的に
算出する関数の戻り値をそのまま`useState`の初期値にしていただけ）。「ライトが
デフォルトに見える」現象は、既存ユーザーに対して明示的に保存された`light`値が
存在するためではなく、単にOS/ブラウザ側の配色設定がライト（またはダーク優先
指定なし）と判定されていたことによるものだった。このため、デフォルト値変更に
あたり既存データの移行は不要だった。

旧実装のコード内コメントには「localStorageが使用不可のため永続化しない」と
あったが、実際には`deviceId.ts`（端末識別子）・`useAuth.tsx`（Supabase-jsの
セッション管理）で既にlocalStorageが使われており、この制約は現状のコードベースの
実態と合っていなかった（Phase 1実装時点の古い制約または誤認と思われる）。
...
1. 予定の置き場所がまだ2箇所に分散（一部解消・一部は残存）
   - training_schedules テーブル（MonthlyCalendar.tsx・Dashboard.tsxで接続済み）
   - training_templates（`training_schedules.template_id`で参照可能）

   **解消済みの部分（2026年8月18〜19日）**：予定作成時にテンプレートを選択すると、
   タイトル・部位が自動入力されるようになった（`ScheduleForm.tsx`の
   `handleTemplateChange`／`buildAutoTitle`）。`Settings.tsx`に
   テンプレート管理UI（`TrainingTemplateManager.tsx`）を新設し、テンプレートの
   新規作成・一覧・削除・「予定を作成」ボタン（該当テンプレートを事前選択した
   状態で新規予定フォームを開く）に対応した。2026年8月19日には、名前・種目構成の
   フル編集にも対応した（`updateTrainingTemplate`、`training_templates`本体を
   更新しつつ`training_template_exercises`は対象テンプレートID分のみ全削除・
   再登録する子テーブル入れ替え方式。全件diff同期ではなく、対象が確定済みの
   1レコード分の入れ替えに限定した安全な範囲）。実績保存時の自動完了判定・
   AI一括取り込み時のtemplateName紐付けも従来通り機能する。

   **残っている部分**：テンプレートと予定はあくまで別テーブル・別概念のままで、
   一元化（例：予定がテンプレートの内容を都度参照するのではなく、テンプレート
   選択時に内容をコピーして予定側に確定させる設計への統合等）はしていない。

   **テンプレート編集時、既存の予定への変更波及は意図的に見送り
   （2026年8月19日、調査の結果による設計判断）**：予定作成時のテンプレート
   自動入力は「作成時コピー確定方式」であり、`training_schedules.title`は
   一度書き込まれると以後テンプレートとは無関係な独立した文字列になる
   （動的参照ではない）。このため、テンプレート編集後にそれを参照している
   既存の予定へ内容を反映させる機能の実装を検討したが、`training_schedules`には
   タイトルが「自動入力のまま」か「ユーザーが手動編集済み」かを区別する追跡列が
   存在せず、実装するとユーザーが意図的に手直ししたタイトルを誤って上書きして
   しまうリスクを排除できないため、見送った（実装漏れではなく調査に基づく
...
4. AI一括取り込みのマスタ名一覧未同梱
   ...
   **実例（2026年8月19日）**：この技術的負債が実際に発生し、2026-08-02〜08-17の
   9日分でトレーニング記録が一部欠落する事象につながった。マスタに未登録の
   種目11件（バイセップ・トライセップ・トーソローテーション等）に加え、
   表記ゆれ3件（「ラット・プルダウン」「クランチ台」「ペクトラルフライ」）が
   フォールバックでも救えずスキップされていた。復旧は手動再取り込みで対応した
   （詳細は上記「2026年8月19日の変更点」内「AI一括取り込みでのトレーニング
   記録欠落」参照）。この際、復旧作業中に新規追加した「シーテッドロー」が
   既存の「シーテッドロウ」と同一種目の重複だったことも後日判明していた。

   **類似度確認ダイアログ機能の実装により一部解消（2026年8月19〜20日）**：
   上記の実例を踏まえ、類似度判定（レーベンシュタイン距離ベース、閾値0.6）による
   確認ダイアログ機能を実装した（詳細は下記「2026年8月19〜20日：AI一括取り込み・
   マスタ登録時の類似名確認ダイアログ機能」参照）。これによりAI一括取り込み時の
   非完全一致名・マスタ新規登録時の表記ゆれ重複は、確認ダイアログ経由で検知・
   防止できるようになった。ただし、プロンプトへのマスタ名一覧未同梱という
