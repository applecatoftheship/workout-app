...
8. 種目0件の実績が「完了」バッジ＋空リストとして表示される
   （2026年8月18日、種目単位削除UI実装時の動作確認で判明。実害小・対応不要と
   判断済み）。`Dashboard.tsx`は、種目単位削除機能によって全種目が削除された
   `training_logs`レコードでも、レコード自体が存在する限り`todayTrainingLogs.length
   > 0`により「今日の運動カード」が「完了」バッジで表示され、種目リストは空になる。
   誤ったデータが表示されるわけではないが見た目がやや不自然。対応する場合は、
   `hasTrainingBlock`判定を`exercises.length > 0`まで見るよう変更する等が考えられる。

9. カレンダーの✅アイコンが、種目全削除後も自動的に「未完了」へ戻らない
   （2026年8月18日、種目単位削除UI実装時の動作確認で判明）。カレンダーセルの
   完了アイコンは`training_schedules.status`（`getScheduleDayIcon`,
...
10. 種目マスタ（exercises）に削除UIが存在しない
    （2026年8月18日、種目単位削除UIの動作確認セッションで判明。テスト用に
    登録した種目マスタ「テスト種目KEEP」「テスト種目DELETE」をUI経由で削除できず、
    手動SQLでの対応が必要だった）。`food_items`には論理削除UI（`GenreFoodPicker.tsx`
    の「削除する食材」ドロップダウン、2026年8月13日実装）があるが、`exercises`には
    同等のUIがない。テスト目的で登録した種目や、誤って登録した種目がマスタに
    蓄積し続ける状態。
