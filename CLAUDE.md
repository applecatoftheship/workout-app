...
1. 予定の置き場所がまだ2箇所に分散（一部解消・一部は残存）
   ...
   **残っている部分**：テンプレートと予定はあくまで別テーブル・別概念のままで、
   ...
   テンプレートを編集する機能もまだない（削除→再作成で代替する運用、
   意図的なスコープ外）。テンプレート編集時に、そのテンプレートを参照している
   既存の予定へ変更が波及する機能もない（そもそもテンプレート編集自体が未実装）。
