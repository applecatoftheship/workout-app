...
### 品質基準・自律修正ループ（Exit Criteria、2026年8月23日）

タスク完了を人間に報告する前に、以下を自動実行すること。
...
2. 5回試行してもグリーンにならない場合はループを止め、試したことと
   エラーログを添えて人間に報告すること（無限に繰り返さない・黙って
   諦めない）。
3. claude-in-chromeによるブラウザ動作確認は、上記「開発方針」の
   「ブラウザでの動作確認はコーディング作業と別セッションで行う」との
   既存ルールを優先する。この自律修正ループのゲートには含めない
   （同一セッション内でコード変更直後にブラウザ確認まで自動連結すると
   既存ルールと矛盾するため）。
4. `vitest`・`build`が両方グリーンになったら、変更内容を要約して人間に
   報告する。**Gitコミット・pushは自動化せず、これまで通り人間の明示的な
   指示を都度受けてから実行する**（本番と開発が同一Supabaseプロジェクトで
   あることに加え、pushは他者から見える状態変更を伴うため、
   都度の確認を省略しない）。
