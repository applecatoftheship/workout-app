...
function BodyPartDrilldownChart({
  dailyVolumes,
  color,
}: {
  dailyVolumes: { date: string; volume: number }[]
  ...
  if (dailyVolumes.length === 0) {
    // 2026年9月18日：同一ファイル内のTrainingBodyPartList（部位別詳細全体の
    // 空状態）と表現を揃え、「この期間の記録はまだありません」に統一。
...
// 進捗グラフ：トレーニング画面 全面刷新v2（2026年8月18日）のセクション3
// 「部位別詳細リスト」。各行を角丸カード化し、左端の色付きボーダーで部位を識別する
// （旧：小さな丸ドットのスウォッチ）。前週比・自己ベストバッジ・ドリルダウン
// （タップで日別ボリューム推移を展開）は旧実装（2026年8月17日）から変更なし。
export function TrainingBodyPartList({ bodyPartVolume }: { bodyPartVolume: BodyPartVolumeEntry[] }) {
  ...
  if (bodyPartVolume.length === 0) {
    return (
      <div className="progress-graph__chart-wrapper">
        <h3 className="chart-card__title">部位別詳細</h3>
      ...
      <ul className="body-part-volume__list">
        {bodyPartVolume.map((item) => {
          ...
          return (
            <li
              key={item.bodyPart}
              className="body-part-volume__item"
              style={{ borderLeftColor: item.color }}
            >
              <button
                type="button"
                className="body-part-volume__row"
                onClick={() => setExpandedBodyPart((current) => (current === item.bodyPart ? null : item.bodyPart))}
              ...
              {isExpanded ? (
                <div className="body-part-volume__drilldown">
                  <BodyPartDrilldownChart dailyVolumes={item.dailyVolumes} color={item.color} />
                </div>
              ) : null}
            </li>
          )
